...
# main release file
release_file <- load_s3("musicbrainz/musicbrainz_release.csv")

# needed for end of collaboration
dates_active <- load_s3("/musicbrainz/mbid_artist_end_date.csv")
dates_active <- dates_active %>% 
  rename(mbz_artist_id = "mbid") %>% 
  as_tibble()

# genre: PLACEHOLDER!
genre <- load_s3("records_w3/items/artists_data.snappy.parquet")
genre <- genre %>% 
  as_tibble() %>% 
  filter(main_genre != "") %>% 
...
df <- release_file %>%  
  ...
  group_by(dz_artist_id, first_release_date_year) %>% 
